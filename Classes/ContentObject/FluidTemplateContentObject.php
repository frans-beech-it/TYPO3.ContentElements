<?php
namespace PatrickBroens\Contentelements\ContentObject;
...
class FluidTemplateContentObject extends AbstractContentObject {
	...
	protected function setTemplate(array $conf) {
		...
		if (!empty($conf['templateName']) && !empty($conf['templateRootPaths.']) && is_array($conf['templateRootPaths.'])) {
			...
			$templateRootPaths = ArrayUtility::sortArrayWithIntegerKeys($conf['templateRootPaths.']);
			...
			foreach ($templateRootPaths as $templatePath) {
				...
				$template = rtrim($templatePath, '/') . '/' . $templateName . '.' . $this->view->getFormat();
				if ($this->testFileExistence($template)) {
